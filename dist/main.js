...
function tracksAreSameSong(a, b) {
    ...
    if (!artistsA.length || !artistsB.length)
        return true;
    return artistsA.some(artistA => artistsB.some(artistB => artistA === artistB || artistA.includes(artistB) || artistB.includes(artistA)));
}
function makeCurrentSpotifyHistoryTrack() {
    if (!lastSpotifyData?.title || !lastSpotifyData?.artist)
        return null;
...
async function preloadSpotifyHistory(force = false) {
    if (!force && spotifyHistoryPreloadPromise)
...
if (closeSpotifyHistoryModalBtn)
    closeSpotifyHistoryModalBtn.addEventListener('click', closeSpotifyHistoryModalFn);
if (spotifyHistoryModal)
    spotifyHistoryModal.addEventListener('click', (e) => { if (e.target === spotifyHistoryModal)
        closeSpotifyHistoryModalFn(); });
window.addEventListener('load', () => {
...
async function getSupabaseAuthClient() {
    ...
    return supabaseGlobal.createClient(SUPABASE_PROJECT_URL, SUPABASE_PUBLISHABLE_KEY, {
        auth: {
            persistSession: true,
...
document.addEventListener('click', async (event) => {
    const target = event.target;
    const button = target?.closest ? target.closest('#floating-trakt-login') : null;
    if (!button)
        return;
    event.preventDefault();
    event.stopPropagation();
