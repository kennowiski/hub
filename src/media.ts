...
function renderSimklData(data) {
    if (!isValidSimklData(data)) return false;

    const posterImg = document.getElementById('simkl-poster') as HTMLImageElement | null;
    ...
    if (ratingSpan) ratingSpan.textContent = renderSimklStars(data.rating);
...
function getSimklStoryButtonHtml(): string {
    return `
...
async function generateSimklStoryBlob(options: { transparentBackground?: boolean } = {}) {
    ...
    const posterEl = document.getElementById('simkl-modal-poster') as HTMLImageElement | null;
    const cardPosterEl = document.getElementById('simkl-poster') as HTMLImageElement | null;
...
async function handleSimklStoryShare(event) {
    ...
    const button = document.getElementById('simkl-story-btn') as HTMLButtonElement | null;
    if (!button) return;
...
function updateSimklModal(data) {
            ...
            const openLink = document.getElementById('simkl-open-link') as HTMLAnchorElement | null;
            ...
            if (modalRatingCaption) modalRatingCaption.classList.toggle('is-visible', hasRating);
    ...
    try {
        ...
        if (!isValidSimklData(data)) return;
...
function ensureOriginalSimklStoryButton() {
    let button = document.getElementById('simkl-story-btn') as HTMLButtonElement | null;
...
const simklCard = document.getElementById('simkl-card');
        ...
        if (simklCard && simklModal) {
            ...
            simklCard.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openSimklModal(); } });
        }
        if (closeSimklModalBtn) closeSimklModalBtn.addEventListener('click', closeSimklModalFn);
        if (simklModal) simklModal.addEventListener('click', (e) => { if (e.target === simklModal) closeSimklModalFn(); });
    ...
    `;
        ...
        document.addEventListener('keydown', (event) => {
            if (event.key === 'Escape') {
                ...
                if (simklModal && simklModal.classList.contains('active')) closeSimklModalFn();
