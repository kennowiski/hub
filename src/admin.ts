...
declare global {
    interface Window {
        ...
        kennyAdminSupabase?: any;
        KENNY_ADMIN_AUTH_ALLOWED?: boolean;
    }
}
    ...
    try {
        ...
        function renderCache() {
            ...
            for (let i = 0; i < localStorage.length; i++) {
                ...
                if (!lower.includes('letterboxd') && !lower.includes('simkl')) continue;
...
document.addEventListener('click', (event) => {
            ...
            const refreshApiButton = event.target.closest('[data-refresh-api]') as HTMLElement | null;
            if (refreshApiButton) {
                refreshApi(refreshApiButton.dataset.refreshApi || '');
                return;
            }

            const clearCacheButton = event.target.closest('[data-clear-cache]') as HTMLElement | null;
            if (clearCacheButton) {
...
(function () {
    ...
    function getDebugElements(key) {
        return {
            card: document.querySelector('[data-debug-card="' + key + '"]') as HTMLElement | null,
            status: document.querySelector('[data-debug-status="' + key + '"]') as HTMLElement | null,
            http: document.querySelector('[data-debug-http="' + key + '"]') as HTMLElement | null,
            time: document.querySelector('[data-debug-time="' + key + '"]') as HTMLElement | null,
            updated: document.querySelector('[data-debug-updated="' + key + '"]') as HTMLElement | null,
            json: document.querySelector('[data-debug-json="' + key + '"]') as HTMLElement | null,
            copy: document.querySelector('[data-debug-copy="' + key + '"]') as HTMLButtonElement | null,
            run: document.querySelector('[data-debug-run="' + key + '"]') as HTMLButtonElement | null
        };
    }
    ...
    function maskSensitiveData(value, parentKey?: string) {
    ...
    function bindDebugEvents() {
        ...
        const runAllButton = document.getElementById('ka-debug-run-all') as HTMLButtonElement | null;
