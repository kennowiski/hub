// script.ts
// Versão em TypeScript do JavaScript extraído do index.html.
// Observação: navegadores não executam .ts diretamente. Para usar no site, compilar para script.js.
...
// Ajuste específico para reduzir o piscar dos modais no navegador interno do Instagram.
        ...
        techBoxes.forEach((box) => {
            const techBox = box as HTMLElement;
            box.addEventListener('mouseenter', (e) => {
                if (window.innerWidth < 900) return;
                document.getElementById('tt-name').textContent = techBox.dataset.name;
                document.getElementById('tt-desc').textContent = techBox.dataset.desc;
                document.getElementById('tt-tag').textContent = techBox.dataset.tag;
                document.getElementById('tt-footer-text').textContent = techBox.dataset.footer;
                document.getElementById('tt-icon').innerHTML = techBox.innerHTML;

                const rect = techBox.getBoundingClientRect();
        ...
        const catGif = document.getElementById('wyd-cat') as HTMLImageElement;
